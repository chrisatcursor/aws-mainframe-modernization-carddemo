...
@Transactional
class TransactionPostingBatchServiceTest {

    @Autowired
    private TransactionPostingBatchService service;
    ...
    @Test
    void processPostingBatch_postsValidAndRejectsOverlimit() {
        ...
        BatchPostingResult result = service.processPostingBatch(LocalDate.now());
        ...
        Account updated = accountRepository.findById(50L).orElseThrow();
        assertThat(updated.getCurrentBalance()).isEqualByComparingTo("125.00");

        TransactionCategoryBalance bal = balanceRepository.findById(key).orElseThrow();
        assertThat(bal.getBalance()).isEqualByComparingTo("25.00");

        assertThat(rejectionRepository.count()).isEqualTo(1);
    }
}
